...
for(i in __:__) {
    ...
    cumulative_scores[__] <- sum(original_answers[1:__])
...
5. Write a function called `feed.me()` that takes a string `food` as an argument, and returns the sentence "I love to eat `food`".
...
6. Try your function by running `feed.me("apples")`, it should return `I love apples`
...
8. Try your `my.mean()` function to calculate the mean age of pirates in the `pirates` data frame (in the yarrr package) and compare your results to what you get from using the `mean()` function.
...
for(i in 1:ncol(__)) {
  ...
  count_over_100 <- sum(data.i > __)
  
  # Print message
  message(paste0("In column ", __, "I found ", __, "values over 100"))

}

```

15. Now, use the following loop to not only count the number of invalid values, but also to *replace* the values with NAs
...
for(i in 1:ncol(__)) {
  ...
  # Print message
  message(paste0("In column ", __, "I found ", __, "values over 100. I will replace them with NA values!"))
