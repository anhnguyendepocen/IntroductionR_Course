...
3. I have a large dataset stored at https://raw.githubusercontent.com/ndphillips/IntroductionR_Course/master/assignments/wpa/data/large_data.txt. The dataset contains responses from 10 participants on 100 problems. Load the data into R as a new object caled `large_data`. 
...
4. None of the participants should have responses larger than 100 on any problem. Using the following loop, count the number of times where each participant gave a value over 100:
...
5. Now, use the following loop to not only count the number of invalid valus, but also to *replace* the values with NAs
...
6. Test to see if your code worked! Try running the same loop again over the dataframe and see what you get!
...
4. Repeat the loop from the previous question, but now, have the loop indicate which **row** the value of 100 is in.
