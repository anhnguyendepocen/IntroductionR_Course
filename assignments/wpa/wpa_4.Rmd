...
# Create demo.df
set.seed(100)
demo.df <- data.frame(id = matthews$id)
...
knitr::include_graphics(path = "../../materials/images/matthewsscreen.png")
```
...
2b. At the top of your script load the `dplyr` package.

```{r}
library(dplyr)
```

3. Now it's time to load the data. The data for this WPA are stored at http://journal.sjdm.org/15/15909/data1.csv. Load the data into R by using `read.table()` into a new object called `matthews` by running the following code.
...
4. Look at the first few rows of `matthews` using `head()`, and `str()` to make sure the data were loaded correctly into R
...
5. Now that you've loaded the data into R, let's save a local copy of the data as a text file called `matthews.txt` into your `data` folder. Using `write.table()`, save the data as a tab--delimited text file called `matthews.txt` in the data folder as follows:

```{r, eval = FALSE, echo = TRUE}
write.table(x = matthews,                 # Save the object matthews
            file = "data/matthews.txt",        # Write the object to study1.txt in the data folder
...
8. Currently gender is coded as 1 and 2. Let's create a new character column called `gender.a` that codes the data as `male` and `female`. Do this by running the following code:

```{r echo = TRUE}
# Create a new column called gender.a that codes gender as a string
matthews$gender.a <- NA  # Start with a column of NA values.
matthews$gender.a[matthews$gender == 1] <- "male"
matthews$gender.a[matthews$gender == 2] <- "female"
```

9. What percent of participants were male?

```{r}
mean(matthews$gender.a == "male")
```

10. Using either basic indexing or `subset()`, calculate the mean age for males only.

```{r}
mean(matthews$age[matthews$gender.a == "male"])

# OR

male.df <- subset(matthews, subset = gender.a == "male")
mean(male.df$age)
```
...
```{r}
mean(matthews$age[matthews$gender.a == "female"])

# OR

female.df <- subset(matthews, subset = gender.a == "female")
mean(female.df$age)
```
...
14. I created a new table containing **fictional** demographic information about each participant. The data are stored in a tab--delimited text file (with a header row) at http://nathanieldphillips.com/wp-content/uploads/2016/10/matthews_demographics.txt. Load the data into an object called  `demo.df` into R.
...
15. Using `merge` add the demographic data to `matthews`
...
16 What was the mean value of `havemore` for each combination of gender and race? (hint: Use `aggregate()` with two independent variables
...
17. Create a new dataframe called `product.df` that only contain columns p1, p2, ... p10 from `matthews` by running the following code. 

```{r, echo = TRUE}
# Create product.df, a dataframe containing only columns p1, p2, ... p10
product.df <- matthews[,paste("p", 1:10, sep = "")]
```
...
20. Add the `pall` vector as a new column called `pall` to the `matthews` dataframe
...
29. Using `save()`, save `matthews`, `gender.df`, `income.df`, `racegender.df`, and `XXX.df` objects to a file called `matthews.RData` in the `data` folder in your working directory.


```{r eval = FALSE}
save(matthews, gender.df, income.df, racegender.df, taskgender.df, file = "data/matthews.RData")
```
