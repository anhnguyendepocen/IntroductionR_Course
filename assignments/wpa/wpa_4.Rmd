...
# Using dplyr
pirates %>%                    # Start with the pirates dataframe ..AND THEN...
  filter(sex == "m") %>%       # Only include male pirates ..AND THEN..
...
# Load the data into R as a new object called matthews
...
matthews$gender_a[matthews$gender == __] <- "__"
matthews$gender_a[matthews$gender == __] <- "__"
```
...
10. What percent of participants were male? (Hint: `mean(__$__ == "__")`)
...
13. Using `aggregate()` (or `dplyr`) calculate the mean age of male and female participants separately using the following template. Do you get the same answers as before?
...
18. Create a new dataframe called `product` that only contain columns p1, p2, ... p10 from `matthews` by running the following code. 
...
21. Add the `pall` vector as a new column called `pall` to the `matthews` dataframe
...
23. Using `aggregate()` or `dplyr` calculate the mean `pall` value for male and female participants separately. Which gender tends to think that others would pay more for products than them?
...
24. Using `aggregate()` or `dplyr` calculate the mean `pall` value of participants for each level of income. Do you find a consistent relationship between `pall` and income?
...
25. Using `dplyr` (and the following template), for each level of gender, calculate the summary statistics in the following table. Save the summary statistics to an object called `gender_agg`
...
26. Using `dplyr`, for each level of `income`, calculate the summary statistics in the following table -- *only for participants older than 21* -- and save them to a new object called `income_df`.
...
27. Using `dplyr`, calculate several summary statistics (you choose which ones!) aggregated at each level of race and gender. Save the results to an object called `racegender_agg`
...
28. Using `dplyr`, calculate several summary statistics (you choose which ones!) aggregated at each level of independent variables of your choice. Save the results to an object called `XXX_df`, where XXX are the names of the variables you aggregated.
...
29. Using `save()`, save `matthews`, `gender_agg`, `income_df`, `racegender_agg`, and `XXX_df` objects to a file called `matthews.RData` in the `data` folder in your working directory.
