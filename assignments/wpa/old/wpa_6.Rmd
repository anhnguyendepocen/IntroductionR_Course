...
1. Do people with higher IQs perform faster on the logic test? Answer this question with a correlation test.
...
```{r}
t.test(partners ~ sex, data = wpa6.df)
```
...
```{r}
cor.test(~ iq + logic, data = wpa6.df)
```
...
Save and email your `wpa_6_LastFirst.R` file to me at nathaniel.phillips@unibas.ch. Then, go to [https://goo.gl/forms/UblvQ6dvA76veEWu1](https://goo.gl/forms/UblvQ6dvA76veEWu1) to complete the WPA submission form.
