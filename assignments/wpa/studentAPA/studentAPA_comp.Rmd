...
Correlations between numeric predictors in the Math data are shown in Figure 2:

```{r fig2, fig.width = 6, fig.height = 6, fig.cap= "Correlations of numeric variables in the math data.", eval = TRUE, out.width = "70%"}
...
papaja::apa_table(portugese.tbl, caption = "Mean Portugese exam scores")
```
...
papaja::apa_table(portugese.tbl, caption = "Mean Math exam scores")
```
...
The t-test on Math exam 1 was non-significant (t(`r sex.por.ttest$parameter`) = `r sex.math.ttest$statistic`, p = `r sex.math.ttest$p.value`), showing no evidence for a difference between men and women on Math exam 1.



```{r}
# G1_aov <- aov(G1 ~ school + sex + absences, data = student.por)
# 
# apa_print(G1_aov)$table
