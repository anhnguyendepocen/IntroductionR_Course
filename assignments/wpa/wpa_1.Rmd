...
We'll start by creating vector objects representing each vector of data from the study.
...
3. Now create the `id` vector again! But this time use the `seq()` function. 
...
12. Oops! It turns out that the watch used to measure time was off. All the before times are 1 second too fast, and all the after times are 1 second too slow. Correct these values! Make sure you also update the `change` vector!
...
R has lots of functions for drawing random samples from probability distributions. For example, you can create random samples of values from a Normal distribution using the `rnorm(n, mean, sd)` function. To illustrate, the following code chunk will save a vector of 50 values from a Normal distribution with mean of 20 and standard deviation of 10
...
31. Run the following code to create a pirateplot of your data, do the results make sense?

```{r, eval = FALSE}
# Put data in a dataframe
sample_data <- data.frame(group = c(rep('10', 10), rep('1000', 1000)),
                          y = c(samp_10, samp_1000))

# Create pirateplot
yarrr::pirateplot(formula = y ~ group, 
                  data = sample_data, 
                  inf.b.o = 0,    # Turn off inference box border
                  inf.f.o = 0)    # Turn off inference box filling

```

32. Now run all of your code for this section again (that is, the code that creates `samp_10` and `samp_1000`, and then plots the results). See how the results change!

## Bonus: The Room with 100 Boxes

Here is a fun little risky decision making game you can program in R using sampling functions. Imagine the following. There is a room with 100 boxes. 99 of the 100 boxes each contain 10 Thousand EUR which you can keep. 1 of the 100 boxes contains a bomb which kills you if you open it.
...
First, create the room as an object `room_100` which ontains a vector with 99 values of 10,000 and one value of negative infinity (`-Inf`) which represents the bomb.
...
```{r}
open <- 0  # How many do you want to open? Change the value from 0 to your number
```
...
You can also represent the boxes game by writing your own custom function in R. Run the following chunk to create the new function `boxes_game`:

```{r}
# Run this chunk to create the function
boxes_game <- function(open, room) {

# Prevents exponent printing in the output
...
Now play the game just by running the function with the number of boxes you want to open as the main argument! For example, here's how you'd play the game by opening 5 boxes.

```{r, eval = FALSE}
boxes_game(open = 5, room = room_100)  # Play with 5 boxes
```

Play the game a few times and see how you do. When you are done, try creating another room called `room_risky` that contains only 10 values: 9 values of 1,000,000 and 1 bomb. Try playing the game in this room a few times and see how your results change.
