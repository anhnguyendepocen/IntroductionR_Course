...
- Generate random samples from probability distributions such as the Normal with `rnorm()`
...
7. Create a vector of the sex data called `sex` but don't use `c()`. Instead, use the `rep()` function by looking for a pattern in the data. To get help on this function, look at the help menu with `?rep`
...
9. Create a new vector called `age_months` that shows the participants' age in months instead of years. (Hint: Just multiply each value by 12)
